...
class Mahalanobis:
    ...
    def pre_normalize(self):
        ...
        if self.normalize_to_unity:
            self.train_embeds_in = self.train_embeds_in / np.linalg.norm(
                self.train_embeds_in, axis=1, keepdims=True
            )
            self.test_embeds_in = self.test_embeds_in / np.linalg.norm(
                self.test_embeds_in, axis=1, keepdims=True
            )
            self.test_embeds_out = self.test_embeds_out / np.linalg.norm(
                self.test_embeds_out, axis=1, keepdims=True
            )
    ...
    def get_metrics(self):
        ...
        self.class_means = [
            np.mean(self.train_embeds_in[self.train_labels_in == c], axis=0)
            for c in range(self.c)
        ]
        self.class_cov_invs = [
            np.cov(
                (
                    self.train_embeds_in[self.train_labels_in == c]
                    - (self.class_means[c].reshape([1, -1]))
                ).T
            )
            for c in range(self.c)
        ]
    ...
    def check_metrics(self):
        ...
        print(
            f"Average distance between cov_inv*cov and identity matrix : {np.mean(np.abs(self.cov_inv.dot(self.cov) - np.eye(self.cov.shape[0])))}"
        )

    def compute_distances(self):
        """Compute Mahalanobis distance for in and out datasets"""
        self.out_totrain = self._maha_distance(
            self.test_embeds_out, self.cov_inv, self.mean
        )
        self.in_totrain = self._maha_distance(
            self.test_embeds_in, self.cov_inv, self.mean
        )

        self.out_totrainclasses = [
            self._maha_distance(
                self.test_embeds_out, self.class_cov_invs[c], self.class_means[c]
            )
            for c in range(self.c)
        ]
        self.in_totrainclasses = [
            self._maha_distance(
                self.test_embeds_in, self.class_cov_invs[c], self.class_means[c]
            )
            for c in range(self.c)
        ]
    ...
    def _maha_distance(
        self, xs: np.ndarray, cov_inv: np.ndarray, mean: np.ndarray
    ) -> np.array:
...
class KLDivergence:
    ...
    def compute_scores(self):
        self.scores = np.concatenate(
            [
                kldivergence(softmax(self.in_logits)),
                kldivergence(softmax(self.out_logits)),
            ]
        )
