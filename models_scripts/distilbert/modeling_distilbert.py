...
class DistilBertForSequenceClassificationHiddenLayer(DistilBertPreTrainedModel):
    def __init__(self, config: PretrainedConfig):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
    ...
    def forward(
        ...
        def create_mean_hidden_states_with_attention(
            hidden_state: list, attentions: list
        ):
            num_examples = hidden_state[0].shape[0]
            outputs_all_ex = torch.empty((num_examples, 6, 768))
            num_head = 0
            for idx_ex in range(num_examples):
                for i in range(6):
                    idx_chosen = int(
                        torch.nonzero(attentions[i][idx_ex][num_head])[-1][1]
                    )
                    outputs_all_ex[idx_ex][i] = hidden_state[i + 1][idx_ex][idx_chosen]
            outputs_all_ex = torch.Tensor(outputs_all_ex.mean(axis=1))
            return outputs_all_ex

        output_hidden_states_mean = create_mean_hidden_states_with_attention(
        ...
        return SequenceClassifierOutput(
            loss=loss,
            logits=logits,
            hidden_states=output_hidden_states_mean,
            attentions=distilbert_output.attentions,
        )


class DistilBertForSequenceClassificationPreLogits(DistilBertPreTrainedModel):
    def __init__(self, config: PretrainedConfig):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.config = config
    ...
    def forward(
        ...
        hidden_state = distilbert_output[0]  # (bs, seq_len, dim)
        pooled_output = hidden_state[:, 0]  # (bs, dim)
        prelogits = self.pre_classifier(pooled_output)  # (bs, dim)
        pooled_output = nn.ReLU()(prelogits)  # (bs, dim)
        pooled_output = self.dropout(pooled_output)  # (bs, dim)
        logits = self.classifier(pooled_output)  # (bs, num_labels)
